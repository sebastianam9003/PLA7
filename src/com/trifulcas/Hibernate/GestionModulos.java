...
public class GestionModulos {
		...
		try {
			...
			if (ListaModu.size() != 0) {
					for(Modulos c: ListaModu) {
								System.out.println(c.getIdmodulo() + " / " + c.getIdprofesor() + " / "+ c.getNombre());
									}
				} else {
	...
	//Metodo para insertar Alumno-modulo en la tabla mod_alu
	public void InsertarAlumnoModulo(int idalumno, int idmodulo) {
		Configuration configuration = new Configuration().configure("hibernate.cfg.xml")
				.addAnnotatedClass(Modulos.class).addAnnotatedClass(Profesores.class).addAnnotatedClass(Alumnos.class);
		StandardServiceRegistryBuilder builder = new StandardServiceRegistryBuilder()
				.applySettings(configuration.getProperties());
		...
		try {
			System.out.println("Insertar un alumno-modulo en la tabla mod_alu, idalumno = " + idalumno + "  idmodulo = " + idmodulo);
			...
			@SuppressWarnings("unchecked")
			List<Alumnos> ListaAlumno = session.createQuery("from Alumnos where idalumno = " + idalumno).getResultList();
						
			// commit de la transacción
			session.getTransaction().commit();
			if (ListaAlumno.size() != 0)
				{
					System.out.println("Existe el alumno, verificando la existencia del modulo.");
					// Crear la configuración cogíendola del xml y añadiendo la clase Categorias
					Configuration configuration1 = new Configuration().configure("hibernate.cfg.xml")
										.addAnnotatedClass(Modulos.class).addAnnotatedClass(Profesores.class).addAnnotatedClass(Alumnos.class);
					StandardServiceRegistryBuilder builder1 = new StandardServiceRegistryBuilder()
							...
							if (ListaModulo.size() != 0) {
								...
								try {
									...
									session2.getTransaction().commit();
										...
										System.out.println("No Existe el modulo con identificador = " + idmodulo);
										System.out.println("No se realiza el alta de Alumno-modulo.");
									   }
						} finally {
									factory1.close();
									}
				} else {
						System.out.println("No Existe el alumnos con identificador = " + idalumno);
						System.out.println("No se realiza el alta de Alumno-modulo.");
					   }
		} finally {
			factory.close();
